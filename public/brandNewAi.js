...
function createAiTable(cfTable, cfColor, skipScnd) {
	...
	cfMoves.forEach(function(stepMove, moveIndex) {
		...
		if (cfRetMoves.length==0){
			...
			var rtnValue=(fHitValue-retHitValue)*10-(rtnHisHitValue-origHisHitValue)+(rtnMyHitValue-origMyHitValue)*100//+(rtnTableValue - origTableValue)*1.1 //+ (rtnMyHitValue - origMyHitValue ) - (rtnHisHitValue - origHisHitValue)//(scndHitValue - origHitValue) +* 10.01
